CLASS ZTP_cl_travel_auxiliary DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*   Type definition for import parameters --------------------------
    TYPES tt_travel_create              TYPE TABLE FOR CREATE   ZTP_i_travel_u.
    TYPES tt_travel_update              TYPE TABLE FOR UPDATE   ZTP_i_travel_u.
    TYPES tt_travel_delete              TYPE TABLE FOR DELETE   ZTP_i_travel_u.

    TYPES tt_travel_failed              TYPE TABLE FOR FAILED   ZTP_i_travel_u.
    TYPES tt_travel_mapped              TYPE TABLE FOR MAPPED   ZTP_i_travel_u.
    TYPES tt_travel_reported            TYPE TABLE FOR REPORTED ZTP_i_travel_u.

    TYPES tt_booking_create             TYPE TABLE FOR CREATE    ZTP_i_booking_u.
    TYPES tt_booking_update             TYPE TABLE FOR UPDATE    ZTP_i_booking_u.
    TYPES tt_booking_delete             TYPE TABLE FOR DELETE    ZTP_i_booking_u.

    TYPES tt_booking_failed             TYPE TABLE FOR FAILED    ZTP_i_booking_u.
    TYPES tt_booking_mapped             TYPE TABLE FOR MAPPED    ZTP_i_booking_u.
    ...
    TYPES tt_bookingsupplement_mapped   TYPE TABLE FOR MAPPED    ZTP_i_bookingsupplement_u.
    TYPES tt_bookingsupplement_reported TYPE TABLE FOR REPORTED  ZTP_i_bookingsupplement_u.


    CLASS-METHODS map_travel_cds_to_db
                            IMPORTING   is_i_travel_u       TYPE ZTP_i_travel_u
                            RETURNING VALUE(rs_travel)      TYPE ZTP_if_flight_legacy=>ts_travel_in.


    CLASS-METHODS map_travel_message
                            IMPORTING iv_cid                TYPE string OPTIONAL
                                      iv_travel_id          TYPE ZTP_travel_id OPTIONAL
                                      is_message            TYPE LINE OF ZTP_if_flight_legacy=>tt_message
                            RETURNING VALUE(rs_report)      TYPE LINE OF tt_travel_reported.



    CLASS-METHODS map_booking_cds_to_db
                            IMPORTING is_i_booking          TYPE ZTP_i_booking_u
                            RETURNING VALUE(rs_booking)     TYPE ZTP_if_flight_legacy=>ts_booking_in.
    ...
    CLASS-METHODS map_bookingsupplemnt_cds_to_db
                            IMPORTING is_i_bookingsupplement      TYPE ZTP_i_bookingsupplement_u
                            RETURNING VALUE(rs_bookingsupplement) TYPE ZTP_if_flight_legacy=>ts_booking_supplement_in.
...
CLASS ZTP_cl_travel_auxiliary IMPLEMENTATION.
